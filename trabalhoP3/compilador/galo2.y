...
#define MAX 1024
    ...
    Variavel procuraDesigDim(char* designacao,int dimensao, Variavel v[], int N){
        int i ,q = 0;
        for(i=0; q<N && i<MAX; i++){
            if(v[i]!=NULL) q++;
            if(strcmp(v[i]->designacao,designacao) == 0 && v[i]->dimensao == dimensao){
                return v[i];
            }
        }
        return NULL;
...
Atrib   : TIPO VAR                              { if(dentrofun){
        ...
        | TIPO VAR '[' NUM ']'                  {  if(dentrofun){
						   	for(contador = 0; contador < $4; contador++){
                                                    		aux = criaVar($1,$2,topol++,contador);
                                                    		insereVar(aux, vl, quantl++);
                                                    		push(aux); push(aux); storel(aux);
                                                  }
                                                  else{
							for(contador = 0; contador < $4; contador++){
                                                    		aux = criaVar($1,$2,topo++,contador);
                                                    		insereVar(aux, v, quant++); 
                                                    		push(aux); push(aux); store(aux);
                                                  } 
                                                }
...
Etipo   : TIPO VAR                              { numelemfun = 1; aux = criaVar($1,$2,-(numelemfun),-1);
                                                  insereVar(aux, vl, quantl++);
                                                }
        | Etipo ',' TIPO VAR                    { numelemfun++; aux = criaVar($3,$4,-(numelemfun),-1);
                                                  insereVar(aux, vl, quantl++);
                                                }
        ;
...
Sexpr   : VAR                                   { if(dentrofun){
                                                    fprintf(out,"pushl %d\n",procuraDesigDim($1,-1,vl,quantl)->posicaoStack); 
                                                  }else{
                                                    fprintf(out,"pushg %d\n",procuraDesigDim($1,-1,vl,quantl)->posicaoStack); 
                                                  }
                                                }
        ...
        | VAR '[' NUM ']'                      {  if(dentrofun){
                                                    fprintf(out,"pushl %d\n",procuraDesigDim($1,$3,vl,quantl)->posicaoStack); 
                                                  }else{
                                                    fprintf(out,"pushg %d\n",procuraDesigDim($1,$3,vl,quantl)->posicaoStack); 
                                                  }
        | Funcao                                { ; } 
